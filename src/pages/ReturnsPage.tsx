...
import { CornerDownLeft, FileText, User, Box, Save, Plus, Undo2, Hash, Banknote } from 'lucide-react';
  ...
  return (
    <section className="page-content pos-container">
      ...
      <div className="pos-header-flex">
        <div>
          <p className="eyebrow" style={{ color: 'var(--fs-blue)', fontWeight: 700, fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>Sales Operations</p>
          <h1 style={{ fontSize: '26px', fontWeight: 800, margin: 0, color: 'var(--fs-text-main)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <CornerDownLeft size={24} color="var(--fs-blue)" />
            مرتجعات المبيعات (Returns)
          </h1>
          <p style={{ marginTop: '4px', color: 'var(--fs-text-muted)', fontSize: '14px' }}>Create offline sales returns and restock products back into inventory.</p>
        </div>
        <button className="pos-action-btn pos-btn-primary" type="button" onClick={() => setShowForm(!showForm)}>
          <Plus size={18} /> {showForm ? 'إلغاء النافذة' : 'تسجيل مرتجع جديد'}
        </button>
      </div>
      ...
      {showForm && (
        <form className="pos-form-card" onSubmit={handleSubmit}>
          <h2 style={{ margin: '0 0 24px 0', fontSize: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Undo2 color="var(--fs-blue)"/> تفاصيل المرتجع
          </h2>

          <div className="pos-form-grid" style={{ marginBottom: 32, paddingBottom: 32, borderBottom: '1px dashed var(--fs-border)' }}>
            <div className="pos-input-group">
              <label><FileText size={16} color="var(--fs-text-muted)"/> الفاتورة الأصلية (Original Invoice)</label>
              <select className="fs-select" value={invoiceId} onChange={(event) => { setInvoiceId(event.target.value); setOriginalItemId(''); }} required>
                ...
                {invoices.map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.invoiceNumber} - {invoice.customerName ?? 'عميل نقدي'}</option>)}
              </select>
            </div>
            <div className="pos-input-group">
              <label><User size={16} color="var(--fs-text-muted)"/> العميل (Customer)</label>
              <select className="fs-select" value={customerId} onChange={(event) => setCustomerId(event.target.value)}>
              ...
              <div className="pos-form-grid">
                <div className="pos-input-group">
                  ...
                  <select className="fs-select" value={originalItemId} onChange={(event) => setOriginalItemId(event.target.value)} required disabled={!selectedInvoice}>
                    ...
                    {selectedInvoice?.items.map((item) => <option key={item.id} value={item.id}>{item.productName} (الكمية: {item.quantity})</option>)}
                  </select>
                </div>
                <div className="pos-input-group">
                  <label><Hash size={16} color="var(--fs-text-muted)"/> الكمية المرتجعة (Quantity)</label>
                  <input className="fs-input" type="number" min="1" step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} />
                </div>
                <div className="pos-input-group">
                  <label><Banknote size={16} color="var(--fs-text-muted)"/> مبلغ الاسترداد (Refund in EGP)</label>
                  <input className="fs-input" type="number" min="0" step="0.01" value={refundCents} onChange={(event) => setRefundCents(event.target.value)} />
                </div>
              </div>
            </div>
            ...
            <div className="pos-input-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <textarea className="fs-input" value={reason} onChange={(event) => setReason(event.target.value)} rows={2} placeholder="سبب الارتجاع (اختياري)..." />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            <button className="pos-action-btn pos-btn-primary" type="submit" disabled={saving || !invoiceId || !originalItemId} style={{ padding: '14px 32px' }}>
              <Save size={18} /> {saving ? 'جاري الحفظ...' : 'تأكيد المرتجع (Create Return)'}
            </button>
      ...
      <div className="pos-table-card">
        <div style={{ padding: '24px 32px', borderBottom: '1px solid var(--fs-border-soft)', background: '#fafafa', display: 'flex', alignItems: 'center', gap: 12 }}>
          <Undo2 size={20} color="var(--fs-text-muted)" />
          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: 'var(--fs-text-main)' }}>سجل المرتجعات (Returns History)</h3>
        </div>
        <table className="pos-cart-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right' }}>
          ...
          <tbody>
            {sortedReturns.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#7a8691', padding: '60px 0' }}>لا توجد مرتجعات مسجلة</td></tr>
              ...
              <tr key={entry.id} style={{ transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--fs-bg)'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <User size={14} color="var(--fs-text-muted)" />
                    {entry.customerName ?? <span style={{ color: 'var(--fs-text-muted)' }}>عميل نقدي</span>}
                  </div>
                </td>
